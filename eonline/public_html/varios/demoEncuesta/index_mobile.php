...
function enviar_variables(){
	...
	if(selected_val>=0 || selected_val<=2)
	{
		...
		$.post("http://la.eonline.com/varios/demoEncuesta/mi_poll_mobile.php",{poll:'1', feed:var_feed, variable:selected_val},function(respuesta){
			//alert(respuesta); //Mostramos un alert del resultado devuelto por el php
			//$("#error").html(respuesta);
			$("#containerPoll").css("display","none");
...
#containerPoll {
	...
	background: #000 url('http://la.eonline.com/varios/demoEncuesta/bg_encuesta.jpg') top center no-repeat;
}
...
<div id="containerPoll">
	<div style="width:240px; height:180px; margin-left:50px;">
		<h1>¿Quién es Ryan Lochte?</h1>
		<ul id="poll">
			<li><input type="radio" name="poll" value="0"/>Un jugador de Fútbol</li>
			<li><input type="radio" name="poll" value="1"/>Un actor</li>
			<li><input type="radio" name="poll" value="2"/>Un nadador</li>
		</ul>
		<input id="bot_poll" type="submit" value="Votar" name="Votar" onclick="enviar_variables();" style="float:right;">
