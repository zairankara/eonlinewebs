					<!-- LIVESTREAMING-->
						...
						$abmLivesRows_url = $mydb2->get_var("SELECT url_mx FROM abmLives WHERE 1");
						...
						if($abmLivesRows==1 && $abmLivesRows_is_category=="home"){?>
							<div style="margin-left:20px; margin-top:10px; margin-bottom:10px; width:620px; height:<?=$height+10;?>px;border:5px solid #000;">
								<iframe src="<?=$abmLivesRows_url?>?f=3" width="615" height="<?=$height?>" scrolling="no" frameborder="0" allowtransparency="allowtransparency"></iframe>
							</div>
