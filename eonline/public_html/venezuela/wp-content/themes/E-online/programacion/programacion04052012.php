...
<div class="col_izq">
	...
	<!-- / Fecha -->
<form method="post" action="http://la.eonline.com/venezuela/programas/" name="ff">
...
<input type="hidden" name="franja" id="franja">
	...
	if($cant==0 || !isset($cant)){
			...
			{
				$id_grid =$row["id_grid"];
				$id_program_press =$row["id_program_press"];
				$ini_bol =$row["ini_bol"];
				...
				$sql4="SELECT * FROM sched_pres WHERE titulo='".$titulo_nuevo."'";
				$resultado4 = mysql_query($sql4);
				if($row4 = mysql_fetch_array($resultado4)){
						$imagen=$row4["id_press"];
						$target="_self";
						if($row4["microsite"]==""){
							$path_file= get_option('siteurl') . "/program/?idPrograma=".$imagen;
						}else{
							$path_file="http://".$row4["microsite"];
							$target="_blank";
						}
				}else{
					$imagen="000";
