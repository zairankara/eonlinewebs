...
class AMP_Content {
	...
	private function transform() {
		...
		////// ABZ add
		preg_match_all('/<blockquote class=\"instagram-media\"[^>]*>(.*?)<\/blockquote>/',$content,$matches_block);
		preg_match_all("#http(s?)://(www\.)?instagr(\.am|am\.com)/p/([^/?]+)#i",$content,$matches_inst);
		preg_match_all('/<blockquote class=\"twitter-(.*?)\"[^>]*>(.*?)<\/blockquote>/',$content,$matches_tweet);
		preg_match_all('#http(s|):\/\/twitter\.com(\/\#\!\/|\/)([a-zA-Z0-9_]{1,20})\/status(es)*\/(\d+)#i',$content,$matches_twitter_url);

		foreach ($matches_inst[0] as $key=>$valor) {
			$content=str_replace($matches_block[0][$key], '[instagram url='.$valor.' hidecaption=true width=320]', $content);
		}

		foreach ($matches_twitter_url[0] as $key=>$valor) {
			$content=str_replace($matches_tweet[0][$key], '[tweet '.$valor.' width=320]', $content);
		}
		////// ABZ add
