	...
	foreach ($arrayMaster as $gallery) 
	{
		...
		*/
		...
		{	
				...
				//exit ("ACA llega!!!: ".$exists."--".$gallery->lastModDate);
	            ...
	            if($wasModified==0)
	            {
				$sqlstr=$EDI_GALL." title='".$titleGallery."', fecha='".$gallery->lastModDate."' WHERE id=$gallery->id";
				$this->executeQuery($sqlstr);
		...
		if($wasModified==0)
        {
				...
				{	
					...
					if($exists=="0")
					{
			    		...
			    		$captionIMG=htmlspecialchars_decode(str_replace("'","\'",$galleryItem->galleryItem->caption));
			    		$captionIMG=str_replace('"',"\'",$captionIMG);
						$sqlstr=$INS_PICT.' ('.$galleryItem->galleryItem->image->id.', "http://images.eonline.com'.$galleryItem->galleryItem->image->filePath."/".$galleryItem->galleryItem->image->source.'","'.$titlePicture.'","'.$captionIMG.'");';
						$this->executeQuery($sqlstr);
