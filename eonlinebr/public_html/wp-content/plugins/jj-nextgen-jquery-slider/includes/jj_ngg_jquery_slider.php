  ...
  function widget($args, $instance)
  {
    ...
    {     
      ...
      foreach($results as $result) 
      {
        ...
        $image_description = trim($image->alttext);                   
        ...
        $vecUrl=explode("http://",$image->imageURL);

        if($vecUrl[2]=="")
        {
                $urlImg=$image->imageURL;
        }else{
                $urlImg="http://".$vecUrl[2];
        }
          
        $output .= "<img src=\"" . $urlImg . "\" " . $image_description . " border=\"0\" class=\"nivo_image\" />";
