...
class WP_Quiz_Page_Config {
	...
	public static function admin_print_styles() {
		...
		wp_enqueue_style( 'semantic-checkbox-css', WP_QUIZ_ASSETS_URL . 'css/checkbox.min.css', array(), time() );
		wp_enqueue_style( 'chosen-css', WP_QUIZ_ASSETS_URL . 'css/chosen.min.css', array(), time() );
	...
	public static function display_messages(){
		...
		if ( isset( $_REQUEST[ 'message' ] ) && ( $msg = (int) $_REQUEST[ 'message' ] ) ) {
			if (  $msg == 3 )
				$message = __('Settings saved', 'wp-quiz');
		}
	...
	public static function load(){
		...
		wp_enqueue_script( 'semantic-checkbox-js', WP_QUIZ_ASSETS_URL . 'js/checkbox.min.js', array( 'jquery' ), WP_QUIZ_VERSION, false );
		wp_enqueue_script( 'chosen-js', WP_QUIZ_ASSETS_URL . 'js/chosen.jquery.min.js', array( 'jquery' ), WP_QUIZ_VERSION, false );
	...
	public static function page(){
		...
		?>
			<div class="wrap" id="config-page">
				<h2><?php _e( 'General Settings', 'wp-quiz' ); ?></h2>
					...
					<div id="poststuff">
						<div id="post-body" class="metabox-holder columns-2">
							...
							<div id="postbox-container-1" class="postbox-container">
								<a href="https://mythemeshop.com/plugins/wp-quiz-pro/?utm_source=WP+Quiz+Free&utm_medium=General+Settings+Banner&utm_content=WP+Quiz+Pro+LP&utm_campaign=WordPressOrg" target="_blank"><img  style="width:100%" src="<?php echo WP_QUIZ_ASSETS_URL . 'image/wp-quiz-pro.jpg' ?>" /></a>
								<a href="https://community.mythemeshop.com/?utm_source=WP+Quiz+Free&utm_medium=General+Settings+Banner&utm_content=WP+Quiz+Pro+LP&utm_campaign=WordPressOrg" target="_blank"><img  style="width:100%" src="<?php echo WP_QUIZ_ASSETS_URL . 'image/have-a-question.jpg' ?>" /></a>
							</div>
							<p class="submit">
								<input type="submit" name="submit" id="submit" class="button-primary" value="<?php _e( 'Save Changes', 'wp-quiz' ); ?>">&nbsp;
	...
	public static function google_analytics_content( $settings ){
		?>
			<div id="analytics_content">
				<label for="profile_name"><?php _e( 'Profile Name', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></label>
				<div class="disabled"><span><input type="text"  id="profile_name" value="" disabled /></span></div>
			
				<label for="tracking_id"><?php _e( 'Tracking ID', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></label>
	...
	public static function default_settings_content( $settings ){
		...
		?>
			<table id="quiz_type_settings" width="100%" frame="border">
				<tr>
					<td><?php _e( 'Restart Questions', 'wp-quiz-' ); ?></td>
					<td>
				...
				<tr>
					<td><?php _e( 'Promote the plugin', 'wp-quiz' ); ?><br/><small><?php _e('Earn 70% commision on every sale by referring your friends and readers.','wp-quiz'); ?></small></td>
					<td>
						<div class="ui toggle checkbox">
				...
				<tr id="quiz_edit_mts_username_row">
					<td><?php _e( 'MyThemeShop username', 'wp-quiz' ); ?><br /><small><?php echo $desc; ?></small></td>
					<td>
						<input style="width:100%;" class="ui" name="defaults[mts_username]" type="text" value="<?php echo esc_attr( $settings['mts_username'] ); ?>" >
					</td>
				</tr>
				<tr>
					<td><?php _e( 'Auto scroll to next question (applies to one page layout)', 'wp-quiz' ); ?><br/></td>
					<td>
						<div class="ui toggle checkbox">
				...
				<tr>
					<td><?php _e( 'Share buttons', 'wp-quiz' ); ?></td>
					<td>
						...
						<select name="defaults[share_buttons][]" id="share_buttons" data-placeholder="None" multiple>
							<option value="fb" <?php echo in_array( 'fb', $settings[ 'share_buttons' ] ) ? 'selected' : '' ?>><?php _e( 'Facebook', 'wp-quiz' ); ?></option>
							<option value="tw" <?php echo in_array( 'tw', $settings[ 'share_buttons' ] ) ? 'selected' : '' ?>><?php _e( 'Twitter', 'wp-quiz' ); ?></option>
							<option value="g+" <?php echo in_array( 'g+', $settings[ 'share_buttons' ] ) ? 'selected' : '' ?>><?php _e( 'Google +', 'wp-quiz' ); ?></option>
							<option value="vk" <?php echo in_array( 'vk', $settings[ 'share_buttons' ] ) ? 'selected' : '' ?>><?php _e( 'VK', 'wp-quiz' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td><?php _e( 'Facebook App ID', 'wp-quiz' ); ?><br/><small><?php printf( wp_kses( __( 'Learn how to create Facebook App ID <a href="%s" target="_blank">here</a>.', 'wp-quiz' ), array(  'a' => array( 'href' => array(), 'target' => array() ) ) ), 'https://youtu.be/MI1QLsDJhJ8?t=3m48s' ); ?></small></td>
					<td>
						<input style="width:100%;" class="ui" name="defaults[fb_app_id]" type="text" value="<?php echo $settings[ 'fb_app_id' ] ?>" >	
					</td>
				</tr>
				<tr>
					<td><?php _e( 'Randomize Questions', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></td>
					<td>
				...
				<tr>
					<td><?php _e( 'Randomize Answers', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></td>
					<td>
						<div class="ui toggle checkbox disabled">
				...
				<tr>
					<td><?php _e( 'Show embed code toggle', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></td>
					<td>
						<div class="ui toggle checkbox disabled">
				...
				<tr>
					<td><?php _e( 'Show Ads', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></td>
					<td>
						<div class="ui toggle checkbox disabled">
				...
				<tr>
					<td><?php _e( 'Show Ads after every nth question', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></td>
					<td>
				...
				<tr>	
					<td><?php _e( 'Countdown timer [Seconds/question]', 'wp-quiz' ) ?><br/><small><?php _e( 'Works only on trivia quiz type when multiple page layout is selected', 'wp-quiz' ) ?></small><br/><small class="pro-text"><?php _e( ' Pro feature', 'wp-quiz' ) ?></small></td>
					<td>
				...
				<tr>
					<td><?php _e( 'Force action to see the results', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></td>
					<td>
						<select  class="ui" id="select" name="">
							<option disabled value="0" ><?php _e( 'No Action', 'wp-quiz' ); ?></option>
							<option disabled value="1" ><?php _e( 'Capture Email', 'wp-quiz' ); ?></option>
							<option disabled value="2" ><?php _e( 'Facebook Share', 'wp-quiz' ); ?></option>
						</select>
					</td>
				</tr>
			</table>
	...
	public static function ad_code_content( $settings ){
		...
		<div id="ad_code_setting">
			<div style="margin-bottom:5px;"><a style="margin-left:0" href="#" id="new_add_code" class="add-new-h2" data-url="https://mythemeshop.com/plugins/wp-quiz-pro/?utm_source=WP+Quiz+Free&utm_medium=Ad+Code&utm_content=WP+Quiz+Pro+LP&utm_campaign=WordPressOrg"><?php _e( 'Add New', 'wp-quiz' ); ?></a><small style="padding-left: 5px;" class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ); ?></small></div>
			<div>
	...
	public static function global_settings_content( $settings ) {
		...
		<table id="global_settings" width="100%" frame="border">
			...
			<tr>
				<td><?php _e( 'Enable or disable players tracking.', 'wp-quiz' ); ?><br/><small class="pro-text"><?php _e( 'Pro feature', 'wp-quiz' ) ?></small></td>
				<td>
					<div class="ui toggle checkbox disabled">
