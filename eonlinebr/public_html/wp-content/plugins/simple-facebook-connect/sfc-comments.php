...
function sfc_comm_login_button() {
	echo '<p><fb:login-button v="2" scope="email,publish_stream" onlogin="sfc_update_user_details();">'.__('Conectar con Facebook', 'sfc').'</fb:login-button></p>';
}
...
function sfc_comm_add_meta($comment_id) {
	...
	if (!empty($_POST['sfc_comm_share']) && !empty($uid) && !empty($token)) {
		...
		$attachment['caption'] = '{*actor*} comentó '.get_the_title($postid);
	...
	FB.getLoginStatus(function(response) {
		if (response.authResponse) {
			...
			if (!jQuery('#fb-user').length) {
				...
				"<span id='fb-msg'><strong><?php echo esc_js(__('', 'sfc')); ?><fb:name uid='loggedinuser' useyou='false'></fb:name>!</strong><br /><?php echo esc_js(__(' Estás conectado con tu cuenta de Facebook .', 'sfc')); ?>" +
				"<a href='#' onclick='FB.logout(function(response) { window.location = \"<?php the_permalink() ?>\"; }); return false;'> <?php echo esc_js(__('Salir', 'sfc')); ?></a>" +
				"</span><span class='end'></span></span>" + 
				"<input type='hidden' name='sfc_user_id' value='"+response.authResponse.userID+"' />"+
				"<input type='hidden' name='sfc_user_token' value='"+response.authResponse.accessToken+"' />");
				jQuery('#sfc_comm_send').html('<input style="width: auto;" type="checkbox" id="sfc_comm_share" name="sfc_comm_share" /><label for="sfc_comm_share"><?php echo esc_js(__('Compartir comentario en Facebook', 'sfc')); ?></label>');
			}
