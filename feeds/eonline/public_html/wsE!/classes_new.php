...
<!-- End of Brightcove Player --><br />';
	...
	if($exists==0)
	{
		$sqlstr=$INS_POST." VALUES (".(int)$post->id.", '".$contentTitle."', '".$contentBody."', 'publish', 1, '".$post->publishDate."', '".$post->publishDate."', 'open', 'closed', '".$friendlyUrl."', 'post', '".$this->convertSAGEDate($post->lastModDate)."');";
		$this->executeQuery($sqlstr);
		//echo("<br>$sqlstr");
		//$this->saveLog("debugging",$sqlstr);	
 ...
 public function MigrateGalleriesData($xmlArray, $importType=NULL)
 {
	...
	foreach ($arrayMaster as $gallery) 
	{
		...
		*/
		...
		$sqlstr_01='DELETE FROM '.PICTURES_TABLE.' WHERE galleryid='.$gallery->id;
                $this->executeQuery($sqlstr_01);

                $sqlstr_02='DELETE FROM '.PICTURES_GALLERIES_TABLE.' WHERE gid='.$gallery->id;
                echo("debug:".$sqlstr_02." rand:".rnd(1,10000));
                $this->executeQuery($sqlstr_02);
