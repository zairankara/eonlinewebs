	...
	foreach ($arrayMaster as $post) {
		...
		foreach ($post->bodySegments as $modulesText)
		{
			...
			$boxContentIMG=(int)$imageArray->payload->sourceWidth+10;
                        $contentBody.="<div class='wp-caption align".$alignIMG."' style='width: ".$boxContentIMG."px;".$notLeftRight."'><IMG SRC='http://images.eonline.com".$imageArray->payload->filePath."/".$imageArray->payload->source."' title='".$imageArray->payload->title."' width='".$imageArray->payload->sourceWidth."' /><p class='wp-caption-text'>".$imageArray->payload->agency."</p></div>";
...
<!-- End of Brightcove Player --><br />';
	...
	//Posts migration -------------------------------------------------------
	$post->publishDate=$this->sumarHoras($post->publishDate, 5);
	...
	if($exists==0)
	{
		...
		case "dev":$term_id_LatinBill="3944";$term_id_EFashion="3832";break;
