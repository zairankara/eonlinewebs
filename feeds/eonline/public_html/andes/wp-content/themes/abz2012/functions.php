...
function meta_alternate(){
	...
	if(is_home()){
		...
		$url_texto=str_replace("/andes/","/",$_SERVER["REQUEST_URI"]);
...
function url_resize_2014($Html_ch, $width_column){
...
/* si es padre o hijo de category wakeup*/
function es_wakeup(){
	$cat = get_term_by('name', single_cat_title('',false), 'category'); 
	$catSlug=$cat->slug;
...
function quien_es_skin(){
		...
		/*$script_div="<div class='skin'>
		<div id='div-gpt-ad-".$postSlug."-160x600-izq' class='skin_izq'>
		<script type='text/javascript'>
		googletag.display('div-gpt-ad-".$postSlug."-160x600-izq');
		</script>
		</div>
		<div id='div-gpt-ad-".$postSlug."-160x600-der' class='skin_der'>
		<script type='text/javascript'>
		googletag.display('div-gpt-ad-".$postSlug."-160x600-der');
		</script>
		</div>
		</div>";*/
...
function rnd_color($random){
	switch ($random){
		...
		case 4 ; 
		case 5 ;
		case 6:$bg_fondo="bg_white";break;
...
function wp_limit_tags_abz($extracto_caption){
	$content = get_the_content();
    $content = str_replace($extracto_caption, "", $content);
	$content = apply_filters('the_content', $content);
    $content = str_replace("brightcove.createExperiences();", "", $content);
