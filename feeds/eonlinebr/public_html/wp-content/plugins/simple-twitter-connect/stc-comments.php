	...
	jQuery(document).ready(function() {
		...
		jQuery.post(ajax_url, data, function(response) {
			if (response != '0' && response != 0) {
				...
				if (!empty($options['comment_text'])) {  // dont do this if disabled 
				?>
				jQuery('#stc_comm_send').html('<input style="width: auto;" type="checkbox" name="stc_comm_send" value="send"/><label for="stc_comm_send">Enviar comentário ao Twitter</label><input type="hidden" id="stc_lat" name="stc_lat" /><input type="hidden" id="stc_long" name="stc_long" />');
...
function stc_comm_get_display() {
	...
	if ($tw) {
		echo "<span id='tw-user'>".
			 ...
			 "<span id='tw-msg'><strong> ".$tw->name."!</strong><br />Você se conectou com uma conta do Twitter. ".
			 "<a href='?stc-logout=1'>Sair</a>".
			 "</span></span>";
		exit;
	}
